...
from omnisafe.adapter.ppo_bc_on_off_policy_adapter import PPOBCOnOffPolicyAdapter
from omnisafe.adapter.ppo_bc_policy_adapter import PPOBCPolicyAdapter
from omnisafe.algorithms import registry
from omnisafe.algorithms.my_algorithms import PPOBinaryCritic
...
"""Just like PPOBinaryCritic, with the difference that:
    - binary_critic is trained as: one step in the environment, one mini-batch"""

@registry.register
class PPOBinaryCriticOnOff(PPOLag):
    ...
    def _init_env(self) -> None:
        self._env: PPOBCOnOffPolicyAdapter = PPOBCOnOffPolicyAdapter(
            self._env_id,
    ...
    def learn(self) -> tuple[float, float, float]:
        """
        Difference with PPOBinaryCritic:
            - take one step in the environment -> run 1 step of mini-batch sgd on binary critic.
        """
        ...
        for epoch in range(self._cfgs.train_cfgs.epochs):
            ...
            for step in track(
                range(self._steps_per_epoch),
                description=f'Processing rollout for epoch: {self._logger.current_epoch}...',
            ):
                ...
                # Take one mini-step on the binary critic
                ...
                # if self._update_count % self._cfgs.algo_cfgs.policy_ == 0:
